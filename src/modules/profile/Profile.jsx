...
const Profile = ({ userRole }) => {
  ...
  return (
    <div className="flex h-screen relative">
          ...
          {tabs.map((tab) => (
            <li
              ...
              className={`p-2 cursor-pointer rounded-lg transition-all font-roboto text-base lg:text-xl ${
                activeTab === tab ? 'text-[#080E52]' : 'hover:bg-gray-200 text-[#D2D6DB]'
              } ${
                tab === 'Logout' ? 'text-red-600 border-t rounded-none border-black mt-2 pt-2' : ''
              }`}
              onClick={() => {
                setActiveTab(tab);
                setIsSidebarOpen(false);
      ...
      {/* Main Content */}
      <div className={`flex-1 p-6 h-screen overflow-y-auto ${isSidebarOpen ? 'hidden lg:block' : 'block'}`}>
        {activeTab === 'Profile' && (
            ...
            <div className="mt-4 flex flex-col lg:px-10">
              ...
              <div className="flex flex-col items-center">
                <div className="bg-[#B190B6] rounded-full w-[114px] h-[114px] flex items-center justify-center">
                  <img src={avatar} alt="User Avatar" className="w-[64px] h-[64px]" />
                </div>
                <h2 className="text-center text-base lg:text-xl text-[#151B3B] font-roboto">USER NAME</h2>
                <button
        ...
        {activeTab === 'Notification' && <Notification onNotificationClick={handleNotificationClick} />}
        {activeTab === 'NotificationDetail' && (
          <NotificationDetail notification={selectedNotification} onBackClick={handleBackClick} />
        )}
